...
import { logInUser } from "../../../apiServices/apiUser";

import TextField from "../../shared/textField";
import Button from "../../shared/button";

import { RESET_PASSWORD_POPUP, REGISTRATION_POPUP } from "../../../constant";

const LogInForm = ({ toggleOpenPopup, logIn }) => {
  ...
  const onSubmit = async (data) => {
    ...
    if (res.error) {
      ...
      if (res.status === 406) {
          toggleOpenPopup("", {email: data.email, needConfirm: true});
      }
    } else {
      toggleOpenPopup("");
      logIn(res);
  ...
  return (
    <form className="form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="form-text">Не имею аккаунта? <button type="button" onClick={toggleOpenPopup.bind(null, REGISTRATION_POPUP)}>Регистрация</button></div>
      <div className="form-row">
        <Button isLink={true} to="https://www.anibelika.com/oauth2/authorization/google" theme="button-white" className="form-btn">Вход с Google</Button>
      </div>
      <div className="form-row">
        <Button isLink={true} to="https://oauth.vk.com/authorize?client_id=7759596&redirect_uri=https://www.anibelika.com&scope=email&display=page&v=5.130&response_type=code" theme="button-white" className="form-btn">Вход с VK</Button>
      </div>
      <div className="form-row">
        <Button isLink={true} to="https://www.anibelika.com/oauth2/authorization/facebook" theme="button-white" className="form-btn">Вход с Facebook</Button>
      </div>
      ...
      <div className="form-text">Забыли пароль? <button type="button" onClick={toggleOpenPopup.bind(null, RESET_PASSWORD_POPUP)}>Восстановить</button></div>
      <div className="form-bottom">
        <Button type="submit" isLoading={isSubmitting} className="form-btn gray">Вход в аккаунт</Button>
      </div>
    </form>
  )
};
